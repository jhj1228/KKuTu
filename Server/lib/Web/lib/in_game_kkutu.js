...
function canSendMove(pos) {
  ...
  if (my.lastMoveAt && now - my.lastMoveAt < PQ_MOVE_INTERVAL) return false;
  if (my.lastMovePos) {
    var dx = Math.abs(pos.X - my.lastMovePos.X);
    var dy = Math.abs(pos.Y - my.lastMovePos.Y);
    if (dx + dy < PQ_MOVE_DISTANCE) return false;
  }
