...
var $data = {};
var $lib = { Classic: {}, Jaqwi: {}, Crossword: {}, Typing: {}, Hunmin: {}, Daneo: {}, Sock: {}, Moqwi: {}, All: {}, Speedquiz: {}, PictureQuiz: {}, Scramble: {}, Wordstack: {} };
var $rec;
var mobile;
...
$lib.Scramble.roundReady = function (data) {
	var tv = L['jqTheme'] + ": " + L['theme_' + data.theme];
...
$lib.Scramble.turnStart = function (data) {
	$(".game-user-current").removeClass("game-user-current");
	$(".game-user-bomb").removeClass("game-user-bomb");
	if ($data.room.game.seq.indexOf($data.id) >= 0) $stage.game.here.show();
	$stage.game.display.html($data._char = data.char);
...
$lib.Scramble.turnGoing = function () {
	var $rtb = $stage.game.roundBar;
	var bRate;
	var tt;
...
$lib.Scramble.turnHint = function (data) {
	playSound('mission');
	pushHint(data.hint);
};
$lib.Scramble.turnEnd = function (id, data) {
	var $sc = $("<div>").addClass("deltaScore").html("+" + data.score);
	var $uc = $("#game-user-" + id);
...
function handleLinkWarning(url) {
	showConfirm(L['linkWarning'], function () {
		window.open(url);
	});
}
	...
	switch (data.type) {
		...
		case 'friendAdd':
			...
			if ($data.opts.df) {
				...
				showConfirm(i + L['attemptFriendAdd'], function () {
					send('friendAddRes', {
						from: data.from,
						res: true
					}, true);
		...
		case 'invited':
			if ($data.opts.di) {
				...
				showConfirm(data.from + L['invited'], function () {
					send('inviteRes', {
						from: data.from,
						res: true
					});
		...
		case 'error':
			...
			if (data.code == 401) {
				...
				// 게임 중
				showConfirm(L['error_' + data.code], function () {
					stopBGM();
					$data._spectate = true;
					$data._gaming = true;
				...
				i = data.message;
				if (i.indexOf("생년월일") != -1) {
					showAlert("생년월일이 올바르게 입력되지 않아 게임 이용이 제한되었습니다. 잠시 후 다시 시도해 주세요.");
					break;
				}
				...
				showAlert("자동화 봇 방지를 위한 캡챠 인증에 실패했습니다. 메인 화면에서 다시 시도해 주세요.");
				break;
			}
...
function addonNickname($R, o) {
	...
	if (o.equip['BDG'] == "b1_subgm") $R.addClass("x-subgm");
}
...
function drawMyGoods(avGroup) {
	...
	renderGoods($("#dress-goods"), 'dress', filter, equip, function (e) {
		...
		if (e.ctrlKey) {
			...
			showConfirm(L['surePayback'] + commify(Math.round((item.cost || 0) * 0.2)) + L['ping'], function () {
			...
			if (item.group == "Mhand") {
				showConfirm(L['dressWhichHand'], function () {
					requestEquip(id, true); // OK = Left hand
				}, function () {
					requestEquip(id, false); // Cancel = Right hand
				});
			} else {
				requestEquip(id, isLeft);
			}
		} else if (item.group == "CNS") {
			showConfirm(L['sureConsume'], function () {
...
function requestEquip(id, isLeft) {
	...
	showConfirm(L[already ? 'sureUnequip' : 'sureEquip'] + ": " + L[id][0], function () {
...
function updateCommunity() {
	...
	function requestRemoveFriend(e) {
		...
		showConfirm(memo + "(#" + id.substr(0, 5) + ")\n" + L['friendSureRemove'], function () {
...
function requestInvite(id) {
	...
	if (id != "AI") {
		...
		showConfirm(nick + L['sureInvite'], function () {
