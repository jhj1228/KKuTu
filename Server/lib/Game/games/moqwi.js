...
exports.roundReady = function(){
	...
	if(my.game.round <= my.round){
		...
		getAnswer.call(my, my.game.theme).then(function($ans){
			...
			$ans.mean = ($ans.mean.length > 20) ? $ans.mean : getVowels($ans._id, Math.round($ans._id.length / 2));
...
exports.turnStart = function(){
	...
	my.game.conso = getVowels(my.game.answer._id, 1);
...
function getVowels(word, lucky){
	...
	for (i = 0; i < len; i++) {
        ...
        if (c < 0 || rv.includes(i)) {
            R += word.charAt(i);
            continue;
        } else c = Math.floor((c % (21 * 28)) / 28);
        R += Const.INIT_VOWELS[c];
    }
	return R;
}
function getHint($ans){
	...
	R.push(h1);
	do{
		h2 = getVowels($ans._id, Math.ceil($ans._id.length / 2));
	}while(h1 == h2);
	R.push(h2);
