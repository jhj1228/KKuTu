...
exports.turnStart = function () {
	...
	my.game.shuffled = shuffleWord(my.game.answer._id);
...
function revealRandomly(word, revealRatio) {
	var len = word.length;
	var revealCount = Math.max(1, Math.floor(len * revealRatio));
	var indices = [];
	var result = '';
	...
	var toReveal = indices.slice(0, revealCount);
	var revealSet = {};
	for (var i = 0; i < toReveal.length; i++) {
		revealSet[toReveal[i]] = true;
	}

	for (var i = 0; i < len; i++) {
		result += revealSet[i] ? word.charAt(i) : '○';
	}
	return result;
}
function getHint($ans) {
	...
	h1 = revealRandomly(word, 0.25);
	R.push(h1);

	h2 = revealRandomly(word, 0.5);
	R.push(h2);
