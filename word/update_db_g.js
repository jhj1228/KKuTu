 ...
 * 한국어 기초사전은 kkutu_ko_g 데이터베이스를 사용합니다.
 */
...
function getFeatVal(featObj, attName) {
    const found = toArray(featObj).find(f => f && f.att === attName);
    return found ? found.val : "";
}

function processJsonData(jsonData) {
    console.log("데이터 정밀 파싱 중...");
    const entries = toArray(jsonData.LexicalResource?.Lexicon?.LexicalEntry);

    const wordMap = new Map();

    entries.forEach((entry) => {
        const rawWord = getFeatVal(entry.Lemma?.feat, "writtenForm");
        if (!rawWord) return;

        const cleanKey = cleanWord(rawWord);
        if (!cleanKey) return;

        const pos = getFeatVal(entry.feat, "partOfSpeech");
        const typeCode = TYPE_MAP[pos] !== undefined ? String(TYPE_MAP[pos]) : "";

        let themeCode = "";
        const semanticCategory = getFeatVal(entry.feat, "semanticCategory");
        if (semanticCategory) {
            const topCat = semanticCategory.split('>')[0].trim();
            if (THEME_MAP[topCat] !== undefined) themeCode = String(THEME_MAP[topCat]);
        }

        toArray(entry.Sense).forEach((sense) => {
            const def = getFeatVal(sense.feat, "definition");
            if (!def) return;
...
async function insertDataToDb() {
    ...
    try {
        ...
        const targetFiles = files.filter(file => file.endsWith(FILE_PREFIX + '.json'));
        ...
        for (let fileIndex = 0; fileIndex < targetFiles.length; fileIndex++) {
            ...
            for (let i = 0; i < processedList.length; i += BATCH_SIZE) {
                ...
                const query = `
                    INSERT INTO public.kkutu_ko_g (_id, mean, type, theme, flag)
                    VALUES 
                    ...
                    DO UPDATE SET 
                        ...
                        flag = COALESCE(public.kkutu_ko_g.flag, EXCLUDED.flag)
                `;
