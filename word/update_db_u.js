 ...
 * 우리말샘은 kkutu_ko_u 데이터베이스를 사용합니다.
 */
...
function processJsonData(jsonData) {
    ...
    items.forEach((item) => {
        ...
        let wordType = item.wordinfo?.word_type || "";
        let senseType = item.senseinfo?.type || "";
        let flagValue = 0;
        if (wordType.includes("외래어")) flagValue |= 1;
        if (senseType.includes("방언")) flagValue |= 8;
        if (senseType.includes("옛말")) flagValue |= 16;
        if (senseType.includes("북한어")) flagValue |= 32;
...
async function insertDataToDb() {
    ...
    try {
        await pool.query(`ALTER TABLE public.kkutu_ko_u ADD COLUMN IF NOT EXISTS flag integer DEFAULT 0`);
        ...
        for (let fileIndex = 0; fileIndex < targetFiles.length; fileIndex++) {
            ...
            for (let i = 0; i < processedList.length; i += BATCH_SIZE) {
                ...
                const query = `
                    INSERT INTO public.kkutu_ko_u (_id, mean, type, theme, flag)
                    VALUES 
